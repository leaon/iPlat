...
doh.register("dojox.dtl.dom.tag",
	[
		...
		function test_tag_extend(t){
			...
			var template = new dd.DomTemplate("{% extends base %}{% block base %}12<ul><li>2a</li><li>2b</li><li>2c</li></ul>3<ul><li>3a</li><li>3b</li><li>3c</li></ul>4<ul><li>4a</li><li>4b</li><li>4c</li></ul>5{% endblock %}");
			t.is("<div>BaseBefore12<ul><li>2a</li><li>2b</li><li>2c</li></ul>3<ul><li>3a</li><li>3b</li><li>3c</li></ul>4<ul><li>4a</li><li>4b</li><li>4c</li></ul>5BaseAfter</div>", dd.tests.dom.util.render(template, context));
		},
...
'{% endfor %}</div>');
		...
		function test_tag_ifequal(t){
			...
			template = new dd.DomTemplate("<div><ul>{% for item in items %}<li><span>{{ item.name }}</span><br/>{{ item.color }}</li>{% endfor %}</ul></div>");
			t.is('<div><ul><li><span>apple</span><br/>red</li><li><span>banana</span><br/>yellow</li><li><span>pear</span><br/>green</li><li><span>kiwi</span><br/>brown</li></ul></div>', dd.tests.dom.util.render(template, context));

			template = new dd.DomTemplate("<div><ul>{% for item in items %}<li>{% ifequal item.name edit_item %}<label>Name: <input type='text' name='name' value=\"{{ item.name }}\"/></label><br/><label>Color: <textarea name='color'>{{ item.color }}</textarea></label>{% else %}<span>{{ item.name }}</span><br/>{{ item.color }}{% endifequal %}</li>{% endfor %}</ul></div>");
			t.is('<div><ul><li><span>apple</span><br/>red</li><li><label>Name: <input name="name" value="banana"/></label><br/><label>Color: <textarea name="color">yellow</textarea></label></li><li><span>pear</span><br/>green</li><li><span>kiwi</span><br/>brown</li></ul></div>', dd.tests.dom.util.render(template, context));

			template = new dd.DomTemplate("<div><ul>{% for item in items %}<li>{% ifequal item.name edit_item %}<div><label>Name: <input type='text' name='name' value=\"{{ item.name }}\"/></label><br/><label>Color: <textarea name='color'>{{ item.color }}</textarea></label></div>{% else %}<div><span>{{ item.name }}</span><br/>{{ item.color }}</div>{% endifequal %}</li>{% endfor %}</ul></div>");
			t.is('<div><ul><li><div><span>apple</span><br/>red</div></li><li><div><label>Name: <input name="name" value="banana"/></label><br/><label>Color: <textarea name="color">yellow</textarea></label></div></li><li><div><span>pear</span><br/>green</div></li><li><div><span>kiwi</span><br/>brown</div></li></ul></div>', dd.tests.dom.util.render(template, context));

			template = new dd.DomTemplate("<div><ul>{% for item in items %}{% ifequal item.name edit_item %}<li><label>Name: <input type='text' name='name' value=\"{{ item.name }}\"/></label><br/><label>Color: <textarea name='color'>{{ item.color }}</textarea></label></li>{% else %}<li><span>{{ item.name }}</span><br/>{{ item.color }}</li>{% endifequal %}{% endfor %}</ul></div>");
			t.is('<div><ul><li><span>apple</span><br/>red</li><li><label>Name: <input name="name" value="banana"/></label><br/><label>Color: <textarea name="color">yellow</textarea></label></li><li><span>pear</span><br/>green</li><li><span>kiwi</span><br/>brown</li></ul></div>', dd.tests.dom.util.render(template, context));
		},
		...
		function test_emptiness(){
			...
			var context = new dd.Context({});
			var template = new dd.DomTemplate('<div>{% if data %}{% else %}Please select a file using the left panel.{% endif %}</div>');
			doh.is('<div>Please select a file using the left panel.</div>', dd.tests.dom.util.render(template, context));
